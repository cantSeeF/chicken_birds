...
void GameLayer9::onExitTransitionDidStart()
{
	setAccelerometerEnabled(false);
}

bool GameLayer9::init(){
	if (!Layer::init()) {
		return false;
	}
	...
	scheduleOnce(schedule_selector(GameLayer9::chickenNotInvicible), 6);
	...
	//2.定义监听对象的回调方法
	listener->onTouchBegan = CC_CALLBACK_2(GameLayer9::onTouchBegan, this);
	listener->onTouchMoved = CC_CALLBACK_2(GameLayer9::onTouchMoved, this);
	listener->onTouchEnded = CC_CALLBACK_2(GameLayer9::onTouchEnded, this);
	listener->onTouchCancelled = CC_CALLBACK_2(GameLayer9::onTouchCancelled, this);
	...
	contactListener->onContactBegin = CC_CALLBACK_1(GameLayer9::onContactBegin, this);
	contactListener->onContactSeparate = CC_CALLBACK_1(GameLayer9::onContactSeparate, this);
...
void GameLayer9::update(float delta){
	//检查是否跑到屏幕外
	getbackToScreen();
	//控制鸟移动
	chickenMoveControl();
...
//触摸开始时调用
bool GameLayer9::onTouchBegan(Touch* touch, Event* event)
{
...
void GameLayer9::getbackToScreen()
{
...
void GameLayer9::chickenMoveControl()
{
...
//碰撞检测
bool GameLayer9::onContactBegin(PhysicsContact& contact){
...
//碰撞检测
void GameLayer9::onContactSeparate(PhysicsContact& contact){
	auto nodeA = (Sprite*)contact.getShapeA()->getBody()->getNode();
	auto nodeB = (Sprite*)contact.getShapeB()->getBody()->getNode();
	...
	if (chickenNode->getPositionY() - birdNode->getPositionY() > 0)//打赢
	{
		for (int i = 0; i < _birds.size(); i++)
		{
			...
			if (enemyT == birdNode) {
				...
				schedule(schedule_selector(GameLayer9::addABird), 3, 3, 3);
	...
	else if (chickenNode->getPositionY() - birdNode->getPositionY() < 0)//打输
	{
		if (_isInvicible == false)
		{
			...
			if (_life > 0)
			{
				...
				scheduleOnce(schedule_selector(GameLayer9::chickenRebirth), 3);
...
void GameLayer9::onAcceleration(Acceleration* acc, Event* event){
	if (_chicken == nullptr){
		return;
	}
...
void GameLayer9::eggSurprise()
{
	...
	case 0:
		_speedStatus = false;
		scheduleOnce(schedule_selector(GameLayer9::getBackBirdSpeed), 10);
		break;
	case 1:
		_isInvicible = true;
		this->getChildByTag(4)->setVisible(true);
		scheduleOnce(schedule_selector(GameLayer9::chickenNotInvicible), 10);
		break;
	case 2:
		if (_life <= 2)
		{
...
void GameLayer9::getBackBirdSpeed(float dt)
{
	_speedStatus = true;
}

void GameLayer9::chickenNotInvicible(float dt)
{
	_isInvicible = false;
	this->getChildByTag(4)->setVisible(false);
}

void GameLayer9::addParticle()
{
...
void GameLayer9::deathBond(Point point)
{
	...
	CallFunc *actionDone = CallFunc::create(bind(&GameLayer9::removeDBond, this));
	auto sequence = Sequence::createWithTwoActions(animate, actionDone);
	dBond->runAction(sequence);
}

void GameLayer9::removeDBond()
{
	this->removeChildByTag(20);
}

void GameLayer9::addWorld()
{
	// Add the ground-down
...
void GameLayer9::chickenRebirth(float dt)
{
	_isInvicible = true;
	this->getChildByTag(4)->setVisible(true);
...
void GameLayer9::addABird(float dt)
{
	if (_birds.size() < 3)
	{
		Sprite *bird;
...
void GameLayer9::addBird()
{
	for (int i = 1; i <= 2; i++)
	{
		auto bird = BlueBird::create();
		if (i == 1)
			bird->setPosition(_origin.x + _visiableSize.width / 2, _origin.y + _visiableSize.height / 2 + 135);
		else if (i == 2)
			bird->setPosition(_origin.x + _visiableSize.width / 2, _origin.y + _visiableSize.height / 2 - 65);
