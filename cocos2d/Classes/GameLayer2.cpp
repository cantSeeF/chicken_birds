...
void GameLayer2::onExitTransitionDidStart()
{
	setAccelerometerEnabled(false);
}

bool GameLayer2::init(){
	if (!Layer::init()) {
		return false;
	}
	...
	scheduleOnce(schedule_selector(GameLayer2::chickenNotInvicible), 6);
	...
	//2.定义监听对象的回调方法
	listener->onTouchBegan = CC_CALLBACK_2(GameLayer2::onTouchBegan, this);
	listener->onTouchMoved = CC_CALLBACK_2(GameLayer2::onTouchMoved, this);
	listener->onTouchEnded = CC_CALLBACK_2(GameLayer2::onTouchEnded, this);
	listener->onTouchCancelled = CC_CALLBACK_2(GameLayer2::onTouchCancelled, this);
	...
	contactListener->onContactBegin = CC_CALLBACK_1(GameLayer2::onContactBegin, this);
	contactListener->onContactSeparate = CC_CALLBACK_1(GameLayer2::onContactSeparate, this);
...
void GameLayer2::update(float delta){
	//检查是否跑到屏幕外
	getbackToScreen();
	//控制鸟移动
	chickenMoveControl();
...
//触摸开始时调用
bool GameLayer2::onTouchBegan(Touch* touch, Event* event)
{
...
void GameLayer2::getbackToScreen()
{
...
void GameLayer2::chickenMoveControl()
{
...
//碰撞检测
bool GameLayer2::onContactBegin(PhysicsContact& contact){
...
//碰撞检测
void GameLayer2::onContactSeparate(PhysicsContact& contact){
	auto nodeA = (Sprite*)contact.getShapeA()->getBody()->getNode();
	auto nodeB = (Sprite*)contact.getShapeB()->getBody()->getNode();
	...
	if (chickenNode->getPositionY() - birdNode->getPositionY() > 0)//打赢
	{
		for (int i = 0; i < _birds.size(); i++)
		{
			...
			if (enemyT == birdNode) {
				...
				schedule(schedule_selector(GameLayer2::addABird), 3,2,3);
	...
	else if (chickenNode->getPositionY() - birdNode->getPositionY() < 0)//打输
	{
		if (_isInvicible == false)
		{
			...
			if (_life > 0)
			{
				...
				scheduleOnce(schedule_selector(GameLayer2::chickenRebirth), 3);
...
void GameLayer2::onAcceleration(Acceleration* acc, Event* event){
	if (_chicken == nullptr){
		return;
	}
...
void GameLayer2::addABird(float dt)
{
	if (_birds.size() < 2)
	{
		auto bird = NormalBird::create();
...
void GameLayer2::eggSurprise()
{
	...
	case 0:
		_speedStatus = false;
		scheduleOnce(schedule_selector(GameLayer2::getBackBirdSpeed), 10);
		break;
	case 1:
		_isInvicible = true;
		this->getChildByTag(4)->setVisible(true);
		scheduleOnce(schedule_selector(GameLayer2::chickenNotInvicible), 10);
		break;
	case 2:
		weatherChange();
		break;
...
void GameLayer2::getBackBirdSpeed(float dt)
{
	_speedStatus = true;
}

void GameLayer2::chickenNotInvicible(float dt)
{
	_isInvicible = false;
	this->getChildByTag(4)->setVisible(false);
}

void GameLayer2::chickenRebirth(float dt)
{
	_isInvicible = true;
	this->getChildByTag(4)->setVisible(true);
...
void GameLayer2::deathBond(Point point)
{
	...
	CallFunc *actionDone = CallFunc::create(bind(&GameLayer2::removeDBond, this));
	auto sequence = Sequence::createWithTwoActions(animate, actionDone);
	dBond->runAction(sequence);
}

void GameLayer2::removeDBond()
{
	this->removeChildByTag(20);
}

void GameLayer2::gameOver()
{
