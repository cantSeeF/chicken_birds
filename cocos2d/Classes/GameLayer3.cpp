...
void GameLayer3::onExitTransitionDidStart()
{
	setAccelerometerEnabled(false);
}

bool GameLayer3::init(){
	if (!Layer::init()) {
		return false;
	}
	...
	scheduleOnce(schedule_selector(GameLayer3::chickenNotInvicible), 6);
	...
	//2.定义监听对象的回调方法
	listener->onTouchBegan = CC_CALLBACK_2(GameLayer3::onTouchBegan, this);
	listener->onTouchMoved = CC_CALLBACK_2(GameLayer3::onTouchMoved, this);
	listener->onTouchEnded = CC_CALLBACK_2(GameLayer3::onTouchEnded, this);
	listener->onTouchCancelled = CC_CALLBACK_2(GameLayer3::onTouchCancelled, this);
	...
	contactListener->onContactBegin = CC_CALLBACK_1(GameLayer3::onContactBegin, this);
	contactListener->onContactSeparate = CC_CALLBACK_1(GameLayer3::onContactSeparate, this);
...
void GameLayer3::update(float delta){
	//检查是否跑到屏幕外
	getbackToScreen();
	//控制鸟移动
	chickenMoveControl();
...
//触摸开始时调用
bool GameLayer3::onTouchBegan(Touch* touch, Event* event)
{
...
void GameLayer3::getbackToScreen()
{
...
void GameLayer3::chickenMoveControl()
{
...
//碰撞检测
bool GameLayer3::onContactBegin(PhysicsContact& contact){
...
//碰撞检测
void GameLayer3::onContactSeparate(PhysicsContact& contact){
	auto nodeA = (Sprite*)contact.getShapeA()->getBody()->getNode();
	auto nodeB = (Sprite*)contact.getShapeB()->getBody()->getNode();
	...
	if (chickenNode->getPositionY() - birdNode->getPositionY() > 0)//打赢
	{
		for (int i = 0; i < _birds.size(); i++)
		{
			...
			if (enemyT == birdNode) {
				...
				schedule(schedule_selector(GameLayer3::addABird), 3, 3, 3);
	...
	else if (chickenNode->getPositionY() - birdNode->getPositionY() < 0)//打输
	{
		if (_isInvicible == false)
		{
			...
			if (_life > 0)
			{
				...
				scheduleOnce(schedule_selector(GameLayer3::chickenRebirth), 3);
...
void GameLayer3::onAcceleration(Acceleration* acc, Event* event){
	if (_chicken == nullptr){
		return;
	}
...
void GameLayer3::eggSurprise()
{
	...
	case 0:
		_speedStatus = false;
		scheduleOnce(schedule_selector(GameLayer3::getBackBirdSpeed), 10);
		break;
	case 1:
		_isInvicible = true;
		this->getChildByTag(4)->setVisible(true);
		scheduleOnce(schedule_selector(GameLayer3::chickenNotInvicible), 10);
		break;
	case 2:
		if (_life <= 2)
		{
...
void GameLayer3::getBackBirdSpeed(float dt)
{
	_speedStatus = true;
}

void GameLayer3::chickenNotInvicible(float dt)
{
	_isInvicible = false;
...
void GameLayer3::deathBond(Point point)
{
	...
	CallFunc *actionDone = CallFunc::create(bind(&GameLayer3::removeDBond, this));
	auto sequence = Sequence::createWithTwoActions(animate, actionDone);
	dBond->runAction(sequence);
}

void GameLayer3::removeDBond()
{
	this->removeChildByTag(20);
}

void GameLayer3::addBird()
{
	for (int i = 1; i <= 3; i++)
	{
...
void GameLayer3::chickenRebirth(float dt)
{
	_isInvicible = true;
	this->getChildByTag(4)->setVisible(true);
...
void GameLayer3::addABird(float dt)
{
	if (_birds.size() < 3)
	{
		Sprite *bird;
