...
Animate* BlueBird::createAnimate(ActionState tobeState) {
...
bool BlueBird::changeState(ActionState state) {
...
void BlueBird::update(float t)
{
	...
	{
		this->getPhysicsBody()->setVelocity(Vect(this->getSpeed() + 100, this->getPhysicsBody()->getVelocity().y));
	}
	else if (direct == 4)
	{
		this->getPhysicsBody()->setVelocity(Vect( - this->getSpeed() - 100, this->getPhysicsBody()->getVelocity().y));
