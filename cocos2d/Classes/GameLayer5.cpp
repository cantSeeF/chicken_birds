...
void GameLayer5::onExitTransitionDidStart()
{
	setAccelerometerEnabled(false);
}

bool GameLayer5::init(){
	if (!Layer::init()) {
		return false;
	}
	...
	scheduleOnce(schedule_selector(GameLayer5::chickenNotInvicible), 6);
	...
	//2.定义监听对象的回调方法
	listener->onTouchBegan = CC_CALLBACK_2(GameLayer5::onTouchBegan, this);
	listener->onTouchMoved = CC_CALLBACK_2(GameLayer5::onTouchMoved, this);
	listener->onTouchEnded = CC_CALLBACK_2(GameLayer5::onTouchEnded, this);
	listener->onTouchCancelled = CC_CALLBACK_2(GameLayer5::onTouchCancelled, this);
	...
	contactListener->onContactBegin = CC_CALLBACK_1(GameLayer5::onContactBegin, this);
	contactListener->onContactSeparate = CC_CALLBACK_1(GameLayer5::onContactSeparate, this);
...
void GameLayer5::update(float delta){
	//检查是否跑到屏幕外
	getbackToScreen();
	//控制鸟移动
	chickenMoveControl();
...
//触摸开始时调用
bool GameLayer5::onTouchBegan(Touch* touch, Event* event)
{
...
void GameLayer5::getbackToScreen()
{
...
void GameLayer5::chickenMoveControl()
{
...
//碰撞检测
bool GameLayer5::onContactBegin(PhysicsContact& contact){
...
//碰撞检测
void GameLayer5::onContactSeparate(PhysicsContact& contact){
	auto nodeA = (Sprite*)contact.getShapeA()->getBody()->getNode();
	auto nodeB = (Sprite*)contact.getShapeB()->getBody()->getNode();
	...
	if (chickenNode->getPositionY() - birdNode->getPositionY() > 0)//打赢
	{
		for (int i = 0; i < _birds.size(); i++)
		{
			...
			if (enemyT == birdNode) {
				...
				schedule(schedule_selector(GameLayer5::addABird), 3, 3, 3);
	...
	else if (chickenNode->getPositionY() - birdNode->getPositionY() < 0)//打输
	{
		if (_isInvicible == false)
		{
			...
			if (_life > 0)
			{
				...
				scheduleOnce(schedule_selector(GameLayer5::chickenRebirth), 3);
...
void GameLayer5::onAcceleration(Acceleration* acc, Event* event){
	if (_chicken == nullptr){
		return;
	}
...
void GameLayer5::eggSurprise()
{
	...
	case 0:
		_speedStatus = false;
		scheduleOnce(schedule_selector(GameLayer5::getBackBirdSpeed), 10);
		break;
	case 1:
		_isInvicible = true;
		this->getChildByTag(4)->setVisible(true);
		scheduleOnce(schedule_selector(GameLayer5::chickenNotInvicible), 10);
		break;
	case 2:
		if (_life <= 2)
		{
...
void GameLayer5::getBackBirdSpeed(float dt)
{
	_speedStatus = true;
}

void GameLayer5::chickenNotInvicible(float dt)
{
	_isInvicible = false;
	this->getChildByTag(4)->setVisible(false);
}

void GameLayer5::addParticle()
{
...
void GameLayer5::deathBond(Point point)
{
	...
	CallFunc *actionDone = CallFunc::create(bind(&GameLayer5::removeDBond, this));
	auto sequence = Sequence::createWithTwoActions(animate, actionDone);
	dBond->runAction(sequence);
}

void GameLayer5::removeDBond()
{
	this->removeChildByTag(20);
}

void GameLayer5::addWorld()
{
	// Add the ground-down
...
void GameLayer5::chickenRebirth(float dt)
{
	_isInvicible = true;
	this->getChildByTag(4)->setVisible(true);
...
void GameLayer5::addABird(float dt)
{
	if (_birds.size() < 3)
	{
		Sprite *bird;
