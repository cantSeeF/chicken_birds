...
void GameLayer8::onExitTransitionDidStart()
{
	setAccelerometerEnabled(false);
}

bool GameLayer8::init(){
	if (!Layer::init()) {
		return false;
	}
	...
	scheduleOnce(schedule_selector(GameLayer8::chickenNotInvicible), 6);
	...
	//2.定义监听对象的回调方法
	listener->onTouchBegan = CC_CALLBACK_2(GameLayer8::onTouchBegan, this);
	listener->onTouchMoved = CC_CALLBACK_2(GameLayer8::onTouchMoved, this);
	listener->onTouchEnded = CC_CALLBACK_2(GameLayer8::onTouchEnded, this);
	listener->onTouchCancelled = CC_CALLBACK_2(GameLayer8::onTouchCancelled, this);
	...
	contactListener->onContactBegin = CC_CALLBACK_1(GameLayer8::onContactBegin, this);
	contactListener->onContactSeparate = CC_CALLBACK_1(GameLayer8::onContactSeparate, this);
...
void GameLayer8::update(float delta){
	//检查是否跑到屏幕外
	getbackToScreen();
	//控制鸟移动
	chickenMoveControl();
...
//触摸开始时调用
bool GameLayer8::onTouchBegan(Touch* touch, Event* event)
{
...
void GameLayer8::getbackToScreen()
{
...
void GameLayer8::chickenMoveControl()
{
...
//碰撞检测
bool GameLayer8::onContactBegin(PhysicsContact& contact){
...
//碰撞检测
void GameLayer8::onContactSeparate(PhysicsContact& contact){
	auto nodeA = (Sprite*)contact.getShapeA()->getBody()->getNode();
	auto nodeB = (Sprite*)contact.getShapeB()->getBody()->getNode();
	...
	if (chickenNode->getPositionY() - birdNode->getPositionY() > 0)//打赢
	{
		for (int i = 0; i < _birds.size(); i++)
		{
			...
			if (enemyT == birdNode) {
				...
				schedule(schedule_selector(GameLayer8::addABird), 3, 3, 3);
	...
	else if (chickenNode->getPositionY() - birdNode->getPositionY() < 0)//打输
	{
		if (_isInvicible == false)
		{
			...
			if (_life > 0)
			{
				...
				scheduleOnce(schedule_selector(GameLayer8::chickenRebirth), 3);
...
void GameLayer8::onAcceleration(Acceleration* acc, Event* event){
	if (_chicken == nullptr){
		return;
	}
...
void GameLayer8::eggSurprise()
{
	...
	case 0:
		_speedStatus = false;
		scheduleOnce(schedule_selector(GameLayer8::getBackBirdSpeed), 10);
		break;
	case 1:
		_isInvicible = true;
		this->getChildByTag(4)->setVisible(true);
		scheduleOnce(schedule_selector(GameLayer8::chickenNotInvicible), 10);
		break;
	case 2:
		if (_life <= 2)
		{
...
void GameLayer8::getBackBirdSpeed(float dt)
{
	_speedStatus = true;
}

void GameLayer8::chickenNotInvicible(float dt)
{
	_isInvicible = false;
	this->getChildByTag(4)->setVisible(false);
}

void GameLayer8::addParticle()
{
	auto parRain = ParticleRain::createWithTotalParticles(100);
	parRain->setVisible(true);
...
void GameLayer8::deathBond(Point point)
{
	...
	CallFunc *actionDone = CallFunc::create(bind(&GameLayer8::removeDBond, this));
	auto sequence = Sequence::createWithTwoActions(animate, actionDone);
	dBond->runAction(sequence);
}

void GameLayer8::removeDBond()
{
	this->removeChildByTag(20);
}

void GameLayer8::addWorld()
{
	// Add the ground-down
...
void GameLayer8::chickenRebirth(float dt)
{
	_isInvicible = true;
	this->getChildByTag(4)->setVisible(true);
...
void GameLayer8::addABird(float dt)
{
	if (_birds.size() < 3)
	{
		Sprite *bird;
